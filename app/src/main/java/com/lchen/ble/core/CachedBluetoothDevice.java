...
public final class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    public void unpair() {
        ...
        if (state == BluetoothDevice.BOND_BONDING) {
            BluetoothDeviceUtils.cancelBondProcess(mDevice);
        }

        if (state != BluetoothDevice.BOND_NONE) {
            final BluetoothDevice dev = mDevice;
            if (dev != null) {
                final boolean successful = BluetoothDeviceUtils.removeBond(mDevice);
                if (successful) {
                    if (Utils.D) {
    ...
    public void setName(String name) {
        if (!mName.equals(name)) {
            mName = name;
            BluetoothDeviceUtils.setAliasName(mDevice, name);
            dispatchAttributesChanged();
        }
    }
    ...
    private void fetchName() {
        mName = BluetoothDeviceUtils.getAliasName(mDevice);//mDevice.getAliasName();
    ...
    void onBondingStateChanged(int bondState) {
        ...
        if (bondState == BluetoothDevice.BOND_BONDED) {
            if (BluetoothDeviceUtils.isBluetoothDock(mDevice)) {
    ...
    public int getConnectionSummary() {
        ...
        for (LocalBluetoothProfile profile : getProfiles()) {
            int connectionStatus = getProfileConnectionState(profile);
            Log.e("info", "connectionStatus :" + connectionStatus);
        ...
        Log.e("info", "profileConnected :" + profileConnected);
        if (profileConnected) {
